...
func PermuteString(input string, matrix []int) string {
	var sb strings.Builder
	for _, val := range matrix {
		sb.WriteByte(input[val-1])
	}
	return sb.String()
}

func InitialKeyPermutation(key string) string {
	return PermuteString(key, PC1[:])
}
...
func TransformKey(permutedKey string, round int) (string, string) {
	...
	rotatedC := utils.RotateLeftString(C_0, numOfRotations)
	rotatedD := utils.RotateLeftString(D_0, numOfRotations)
	nextKeyBits := rotatedC + rotatedD

	subKey := PermuteString(nextKeyBits, PC2[:])
	return subKey, nextKeyBits
}

func sBoxLookup(slice6Bits string, boxIndex int) string {
	rowBinary := string(slice6Bits[0]) + string(slice6Bits[5])
	colBinary := slice6Bits[1:5]

	row, _ := strconv.ParseInt(rowBinary, 2, 64)
	col, _ := strconv.ParseInt(colBinary, 2, 64)

	val := sBox[boxIndex][row*16+col]
