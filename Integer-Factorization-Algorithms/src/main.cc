 ...
 * @brief Call and execute the different functions from this project.
 * @license GPLv3
 */
...
int main() {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1: {
                TrialDivision td;
                result = td.ComputeTrialDivision(target);
                break;
            }
            case 2: {
                Fermat fermatSolver;
                result = fermatSolver.ComputeFermat(target);
                break;
            }
            case 3: {
                PollardsRho rhoSolver;
                result = rhoSolver.ComputePollardsRho(target);
                break;
            }
        }
        std::cout << "------------------------------------\n";
